---
title: "Documentation for the SRN Data Dashboard"
author:
    - name: Sustainability Reporting Navigator
      url: https://srnav.com
date: today
date-modified: today
description: "Technical documentation of the ESG indicator extraction pipeline underlying the Sustainability Reporting Navigator's ESG performance dataset."
lang: en
format:
  pdf:
    include-in-header:
...
link-citations: true
abstract: |
  This whitepaper documents the automated ESG indicator extraction pipeline
  of the Sustainability Reporting Navigator (SRN). The pipeline retrieves
...
The *transparency* dimensions captured by the SRN dataset describe **how** firms report rather than **what** they report. They are derived from the same annual and sustainability reports used for the quantitative ESG indicators (see [Performance]), but rather than extracting individual numbers, we measure textual properties of each document — its length, readability, topic coverage, and the specificity with which it discusses environmental and social issues. Reports are first parsed with Docling [@docling2024] and then processed with Python at the sentence level. The downstream language model used to classify environmental and social passages was originally developed for @Donau2024-yc; we adopt their label set and decision thresholds without modification, and refer the reader to that paper for full methodological detail.
